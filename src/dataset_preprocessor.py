"""
Dataset Preprocessor for SourceSleuth — arXiv Metadata Pipeline.

Handles the preprocessing of the Kaggle arXiv metadata snapshot
(``arxiv-metadata-oai-snapshot.json``, ~5 GB, ~3 M records) into a
compact, cleaned format suitable for embedding and semantic search.

Dataset Preprocessing Documentation (Hackathon AI/ML Requirement):
    - **Source**: Kaggle arXiv Dataset (https://www.kaggle.com/Cornell-University/arxiv)
    - **Format**: JSON-Lines (one JSON object per line)
    - **Original size**: ~5 GB, ~2.97 M records
    - **Preprocessing steps**:
        1. Stream-read line-by-line (memory-efficient, no full load)
        2. Extract relevant fields: id, title, authors, abstract,
           categories, doi, journal-ref, update_date
        3. Clean text: strip LaTeX artifacts, normalise whitespace,
           remove arXiv preamble indentation from abstracts
        4. Optionally filter by arXiv category (e.g. cs.AI, cs.CL)
           and/or date range to create focused subsets
        5. Output a smaller preprocessed JSON-Lines file + stats
    - **Rationale**: Full corpus is too large to embed on a student
      laptop. Filtering to relevant categories (e.g. CS, Physics)
      keeps the working set manageable (< 500 k records) while
      retaining high relevance for academic citation recovery.
"""
...
@dataclass
class ArxivRecord:
    """A cleaned, minimal representation of an arXiv paper."""
...
def preprocess_dataset(
    ...
    """
    Preprocess the raw arXiv metadata into a cleaned JSON-Lines file.

    Args:
        input_path:  Path to the raw ``arxiv-metadata-oai-snapshot.json``.
        output_path: Path for the cleaned output file.
        categories_filter: Exact arXiv categories to include.
        category_prefix_filter: Category prefixes to include (e.g. ``cs.``).
        start_date: Only include records updated on/after this date.
        max_records: Maximum number of records to output.

    Returns:
        PreprocessingStats with counts and category distribution.
    """
